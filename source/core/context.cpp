...
{
    cl_int err;
    std::vector<cl_context_properties> context_props = props;
    context_props.push_back(0);
    _context = clCreateContext(context_props.data(), 1, &_device, nullptr, nullptr, &err);
    if (err != CL_SUCCESS || !_context) {
        throw std::runtime_error("Failed to create OpenCL context.");
    }
    _queue = clCreateCommandQueue(_context, _device, 0, &err);
    if (err != CL_SUCCESS || !_queue) {
